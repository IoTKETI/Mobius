...
- ~~**`my.cnf` 정리**~~ — 2026-09-02 불필요해졌다. `max_connections` 는 기동
  검사가 바닥까지 올리므로 파일이 낮아도(300) 안전하다. 5번 참고.
...
#### `max_connections` 바닥 검사 (2026-09-02, 완료·배포·실측)

`max_connections` 만 주인을 마이그레이션에서 **기동 검사**로 옮겼다.
나머지 셋은 010 이 한 번만 넣는다.

가른 기준은 **기본값이 위험한가** 하나다:

| 값 | MySQL 기본 | 유실되면 |
|---|---|---|
| `innodb_flush_log_at_trx_commit` | 1 | 원하는 값 그대로 |
| `transaction_isolation` | REPEATABLE-READ | 원하는 값 그대로 |
| `sync_binlog` | 1 | 느릴 뿐 안전 |
| **`max_connections`** | **151** | **앱이 625 를 요구 → 즉시 고갈** |

마이그레이션은 한 번 돌고 `schema_migrations` 에 기록되므로 `SET PERSIST`
유실(DB 복구, `RESET PERSIST`)을 못 고친다. 유실은 언제나 마이그레이션을
다 적용한 뒤에 온다. 그래서 이 값만 기동마다 본다.

바닥은 박지 않고 계산한다 — `mobius/pool_sizing.js`:

```
dbConnectionLimit x (워커 + 마스터) x 1.2, 100 단위 올림
25 x 25 = 625 -> 750 -> 800
```

`dbConnectionLimit` 을 바꾸면 바닥이 따라온다. 800 을 상수로 두면 그 둘이
갈린다. 설정 화면은 `describe()` 의 `derived`(processes / slack / roundTo)
로 같은 계산을 해서 입력값 옆에 필요량을 그릴 수 있다.

**올리기만 한다.** 지금 값이 바닥 이상이면 안 건드린다 — 기동마다 2000 으로
덮어쓰던 옛 `set_tuning` 과 갈리는 지점이 이것이다.

배포 실측(2026-09-02):

```
800 -> 재기동 -> 800        (건드리지 않음. 로그도 조용하다)
300 -> 재기동 -> 800        [db_bootstrap] max_connections 300 < 필요 800
                            (풀 25 x 프로세스 25) — 올린다
                            mysqld-auto.cnf 에 800 으로 기록됨
```

**첫 배포는 검사가 한 번도 안 돌았다.** 검사를 "적용할 마이그레이션이
있을 때" 안에 두었는데, 배포된 서버는 010 까지 다 적용돼 `pending` 이 0 이라
그 분기에 들어가지도 못했다 — 검사가 필요한 상태에서만 정확히 안 돌았다.
소스를 정규식으로 보는 검사 900개가 전부 통과했다. "무엇을 하는가" 는 봐도
"언제 도는가" 는 못 보기 때문이다. 지금은 가짜 파사드 위에서 실제로 돌리는
검사가 둘 있다(`test/db-bootstrap.test.js`).

#### 남은 것

관리 콘솔의 DB 설정 화면은 `mobius-fd` 가 계획을 세웠고
(`2026-09-01-db-settings-console-plan.md`) 사용자가 직접 진행한다.

### 5. `my.cnf` 정리 — **불필요해짐 (2026-09-02)**

`mysqld.cnf` 는 root 파일이라 관리 UI 가 손댈 수 없었고, 그래서
"설치 가이드에 `max_connections` 를 800 으로 고치라고 적을 것인가" 가
남아 있었다. 바닥 검사가 그 자리를 대신한다 — 새로 설치한 서버가 기동만으로
필요한 값이 된다. 파일은 그대로 둔다.

파일과 실제가 다른 것 자체는 남는다(`mysqld.cnf:100` 이
`innodb_flush_log_at_trx_commit = 1`, 적용값도 지금은 1 이라 같다).
`max_connections` 만 파일 300 / 적용 800 인데, **파일이 낮은 쪽이라 위험하지
않다** — 기동 검사가 언제나 위로 끌어올린다.
