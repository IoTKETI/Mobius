...
## 7. `/hit` · `/total_ae` · `/total_cbs` 의 노출 (2026-09-06 추가)

요청 흐름 남은 일(`docs/superpowers/specs/2026-09-04-request-flow-remaining.md`)
§3.4(나) 에서 넘어왔다. **코드는 바꾸지 않았다** — 어떻게 막을지가 배포·관리
결정이라 관리 UI 작업에서 같이 정하기로 했다(2026-09-06).

### 지금 어떻게 되어 있나

`app.js` 의 `extra_api_action` 이 세 URL 을 `check_xm2m_headers` **보다 먼저**
받는다(호출부 `app.js:2534`). 그래서 `X-M2M-RI` 도 `X-M2M-Origin` 도 없이
GET 하면 200 과 JSON 이 돌아오고, ACP 도 거치지 않는다. **일부러 그렇게
만든 것이다** — Mobius 상태·리소스 현황을 보려고 따로 둔 경로라 헤더 검사를
뺐다.

| URL | 질의 함수 | 새는 것 |
|---|---|---|
| `/hit` | `db_sql.get_hit_all` | 일자별 프로토콜 호출 건수 — `hit` 테이블 전체 |
| `/total_ae` | `db_sql.select_sum_ae` | AE 총수 |
| `/total_cbs` | `db_sql.select_sum_cbs` | CIN 바이트 총합 |

- 입력 파라미터가 **0개**다(`url ==` 정확 일치, 질의문 고정). 주입면은 없다
- **외부에서 닿는다** — 배포 서버 밖에서 `http://<호스트>:7579/hit` 이 200 으로
  응답하는 것을 확인했다(2026-09-04). 자격증명도 리소스 내용도 아니지만
  **운영 규모가 그대로 드러난다**
- 정산기 우회는 이미 고쳤다(§3.4(가), 2026-09-04). `settle.raw('extra api …')`
  가 응답을 보낸 뒤 반납하고 이중 정산 방지도 탄다. 로컬 120회 연속 호출에
  매달림 없음
- `extra_api_action` 안에 `if (0)` 블록 둘이 남아 있다(`hit.json` 백업
  가져오기 · 무작위 hit 생성기, `app.js:1878-1938`). 도달하지 않는 코드다.
  이 경로를 옮기거나 지울 때 같이 걷는다

### 결정할 것

셋 중 하나다. **첫째를 권한다.**

1. **방화벽·관리망으로 막는다** — 코드 변경이 없다. 7579 포트의 이 세 경로만
   사내망에서 열면 된다. 배포 설정이라 저장소에 남는 것이 없으니 **어디에
   무엇을 걸었는지는 이 문서에 적어 둔다**
2. **관리 콘솔로 옮긴다** — 콘솔(`admin/`)은 이미 코어 파사드(`mobius/db/`)로
   DB 를 읽으므로 같은 세 질의를 콘솔 화면에 두고, `app.js` 의
   `extra_api_action` 과 호출부(`app.js:2534`, `settle.raw('extra api …')` 갈래)를
   지운다. 이때 손볼 시험은 `test/settle-helper.test.js` 의 "라우트 핸들러
   안에서 db.release 를 직접 부르지 않는다"(이 경로를 예외로 안다) 와
   `test/converted-queries.test.js` · `test/db-mysql2-contract.test.js`(세 질의
   함수를 이름으로 안다) 다. 세 URL 을 부르던 외부 도구가 있으면 끊긴다
3. **헤더 검사 뒤로 옮긴다** — `X-M2M-Origin` 을 요구해 수퍼유저만 받게 한다.
   코드 변경이 가장 작지만 조회 도구마다 마스터 키를 심게 되므로 권하지
   않는다

### 이 작업과 무관하다는 것

세 URL 의 응답(200 + `application/json`, 본문은 질의 결과 그대로)은 oneM2M
리소스 응답이 아니라 `settle.raw` 로 직접 나간다(`app.js:2563-2580` 주석).
응답 구조 재작성(`responder.respond`)과 얽히지 않는다.
