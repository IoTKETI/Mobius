...
### 정정 (2026-08-27, 인자 버그 수정 후) — 감소 경로는 **배선돼 있었다**

위 표에서 "CIN 단건 DELETE → 없음" 이라고 적었는데, **틀렸다.**

`resource.js` 의 `delete_action` 안에 이미 감소 경로가 있다:

```js
else if (resource_Obj[rootnm].ty == '4') {
    update_cnt_by_delete(request.db_connection, resource_Obj[rootnm].pi, /* cs 누락 */ function (rsc) {});
    callback('200');
}
```

`update_cnt_by_delete(connection, pi, cs, callback)` 는 `pi` 로 부모를 조회한 뒤
`update_parent_by_delete` 를 부른다 — 설계는 정확했다. 다만 **호출부가 `cs` 를 빠뜨려**
콜백 함수가 `cs` 자리에 들어갔고, 그래서 매번 실패했다.

`cs` 를 제대로 넘기도록 고치자 **두 백엔드 모두 카운터가 맞았다:**

| 모드 | 삭제 전 | 삭제 후 | 실제 데이터 |
|------|--------|--------|-----------|
| SQLite | `cni=2, cbs=10` | **`cni=1, cbs=4`** | `n=1, total=4` |
| MySQL | `cni=2, cbs=10` | **`cni=1, cbs=4`** | `n=1, total=4` |

**따라서 아래 안 A/B/C 는 "버그 수정" 이 아니라 "성능 최적화" 로 성격이 바뀐다.**
지금은 삭제마다 `update_parent_by_delete` 가 즉시 돈다 (= 아래 **안 B** 상태).
...
**당장은 아무것도 하지 않는 것을 권한다.** 인자 버그를 고친 지금이 안 B 상태이고,
정확성 문제는 해소됐다.

안 A 로 옮기는 것은 **삭제가 실제로 병목일 때** 하면 된다. 판단 기준:

- IoT 워크로드에서 CIN 삭제는 삽입보다 훨씬 드물다. 삽입은 초당 수십~수백 건이지만
  단건 삭제는 사람이나 관리 도구가 부르는 경우가 대부분이다.
- 대량 삭제는 이미 `delete_oldest` 보존 정책 purge 가 **일괄 감산**으로 처리한다.
  거기는 이미 배치다.
- 따라서 안 A 가 줄여 줄 UPDATE 횟수는 크지 않을 가능성이 높다.

먼저 측정하고 나서 정하는 편이 낫다 — `update_parent_by_delete` 의 `console.time` 이
이미 붙어 있으므로 로그로 호출 빈도와 소요를 볼 수 있다.

안 C 는 A/B 와 배타적이지 않다 — 어느 쪽이든 안전망으로 둘 만하다.
