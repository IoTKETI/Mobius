...
### 2.2 워커의 maxSockets 가 1,000,000 이다  — 성격이 달라졌다

```
app.js:650 · 667    워커 (http · https)
app.js:710 · 722    죽은 단일 프로세스 분기
```
...
남은 넷은 **워커** 것이라 위험도가 다르다 — 워커는 죽으면
`cluster.on('exit')` 이 다시 띄우고, 유입도 HTTP 라 커넥션 수가 역압으로
작동한다. **급하지 않다.** 다만 1,000,000 이라는 값에 근거가 없다는 점은
그대로다.
