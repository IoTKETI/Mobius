...
### 1.1 ~~DB 커넥션을 못 얻으면 포트를 안 열고 그냥 산다~~ — **고쳤다** (`47e3a7e`, 2026-09-05)

`fail_start(역할, 이유)` 가 마스터·워커·단일 프로세스 셋 다에서 종료해 pm2 가 다시 띄운다
(`app.js` 의 `db.connect` 실패 갈래 셋). 이 문서의 제목만 갱신이 빠져 있었다(2026-09-06 확인).
아래는 고치기 전 서술이다.
...
### 2.2 ~~워커의 maxSockets = 1,000,000~~ — **지웠다** (2026-09-04, `26bc169`)

네 줄을 `26bc169` 에서 지웠고 같은 날 배포됐다(배포 HEAD 의 조상이다). `test/outbound-timeout.test.js` 의
"globalAgent.maxSockets 를 손으로 세우지 않는다" 가 app.js·fanout·fopt·grp·sgn_man 에 되살아나는 것을 막는다.
이 절은 2026-09-07 까지 "고치지 마라(지금은)" 로 남아 있었고, 그 서술만 보고 사용자에게 "보류" 라고 잘못
보고했다 — 코드가 아니라 문서를 보고 답한 실수다. 사용자 지시("그냥 지워", 2026-09-07)로 코드를 보니 이미 없었다.
죽은 단일 프로세스 분기(`use_clustering = 1` 의 else, app.js:947)는 그대로 있다 — 이 절의 대상이 아니다.
아래는 고치기 전 서술이다.
...
`mgo` 쪽 관문(`405-15`, 옛 `409-4`)도 의도된 것이다 — 그 자리 주석이 "mgo 는 추상
...
### 3.4 `/hit` · `/total_ae` · `/total_cbs` 가 검증도 ACP 도 안 거친다 — **(가) 고쳤다 · (나) 관리 UI 작업으로 넘겼다** (2026-09-06)
...
**(나) 인증·노출 — 관리 UI 작업으로 넘겼다** (2026-09-06). 코드는 안
바꿨다. 사실과 선택지(방화벽 · 콘솔로 이전 · 헤더 검사 뒤로)는
`docs/superpowers/specs/2026-08-29-admin-ui-handoff.md` **7절**에 있고,
결정은 그 작업에서 한다. 아래는 넘기기 전의 서술이다.

사용자가 Mobius 상태·리소스 현황을 보려고 따로 만든 것이라 `X-M2M-RI`·
...
### 5.2 ~~팬아웃이 직렬이라 최악 대기가 곱해진다~~ — **상한 있는 병렬로** (2026-09-05)

`mobius/fanout.js`(새 파일)가 멤버 요청을 동시에 **8개**까지 띄우고 하나가
끝나면 다음을 띄운다. 최악 대기는 N×10초에서 ⌈N/8⌉×10초. 결과는 멤버 순서
배열에 받아 두었다가 mid 순서로 `agr` 에 넣으므로 응답 본문은 직렬일 때와
바이트 같다. 실패 방침(멤버 하나의 타임아웃·비JSON·연결 실패는 그 멤버만
제외, 경로 모르는 원격은 건너뜀)은 그대로다.

**상한 8 의 근거.** 멤버는 대개 이 CSE 자신(localhost)이라 동시 N 개가 곧 이
클러스터의 워커·DB 커넥션 N 개다. 배포 실측(2026-09-05): 그룹 5개, 최대 멤버
4, 평균 2.8, 오늘 fopt 5건(p50 69ms) — 전부 상한 아래라 오늘은 완전 병렬이고,
`mnm` 밖에 멤버 수 상한이 없어 큰 그룹이 생겨도 요청 하나가 8개 이상 잡지
못한다. conf 키로 빼지 않았다(조정할 실측이 없다).

**멤버별 DB 조회도 없앴다.** 옛 `fopt_member` 는 `check` 가 목록을 푼 뒤에도
멤버마다 `get_ri_sri` 를 또 불렀는데 헛돈 게 아니었다 — `make_internal_ri` 가
첫 조회 **뒤에** 돌아서 'Mobius/…' 형은 두 번째 조회가 풀었다. 그리고 `lookup`
은 **`ri` 가 구조 경로, `sri` 가 짧은 id** 다(배포 실측; 문서 초안은 반대로
알고 있었다). 원값과 접은 값 중 어느 쪽이 `sri` 에 맞을지 미리 알 수 없으므로
둘을 합쳐 `whereIn` **한 번**에 묻고 옛 우선순위(원값 적중 → 접은 값 적중 →
접은 값)로 고른다. 배포 그룹 5개의 mid 는 전부 'Mobius/…' 형이라 접은 값 쪽.

**왜 새 파일인가.** `fopt.js` 는 `resource.js` 를 통해 `sgn_man` 까지 끌어와
require 만으로 MQTT 클라이언트가 열린다. `fanout.js` 는 DB·전역을 모르므로
시험이 실물 HTTP 멤버 서버로 돈다.

증명: `test/fopt-parallel.test.js` 10건(서버가 관측한 최대 동시 요청 = 8,
순서 보존, 침묵·비JSON·경로 없는 멤버 제외, 소스 규칙) · 변이 12건 전부 잡힘 ·
팬아웃 골든 14건 전후 동일(혼합 표기 그룹 — cse 상대·짧은 id·없는 멤버 — 의
멤버 순서와 풀린 경로 포함). **부작용**: 이제 팬아웃도 `get_ri_list_sri` 를
지나므로 §5.3 의 치환 로그가 `/fopt` URL 로도 찍힌다 — ACP 경로 판단에서는
그 줄을 빼고 본다. 아래는 고치기 전 서술이다.
...
- ~~**rsc↔HTTP 불일치**~~ — 교정했다(2026-09-06). 규칙 둘: **한 rsc 는 한 HTTP**
  (TS-0009 매핑, 예외는 CONTENT_TOO_LARGE 4000/413 하나) · **사유 키 접두 = HTTP**.
  `CONFLICT_OPERATION`(4005/409)을 없애고 la/ol 에 POST·PUT(409-1·2)과 미지원
  리소스(409-4)는 405-13~15 로, 예약어 rn(409-3)은 400-66 BAD_REQUEST 로. 포워딩
  불가(301-3·4)는 501-3·4 NOT_IMPLEMENTED, poa 없음(301-5)은 새 항목
  `TARGET_NOT_REACHABLE` 5103/404(404-9)로. 탐색 상한(500-6)은 값 그대로 키만 400-67.
  배포 최근 3일 실측: 405 0 · 501 0 · 409 68(전부 4105 "이미 있음") — 여덟 사유가
  한 번도 나가지 않아 계약 변화의 노출은 없다. `test/reason-catalog.test.js` ·
  `test/rsc-catalog.test.js` 가 규칙 둘을 잠근다
