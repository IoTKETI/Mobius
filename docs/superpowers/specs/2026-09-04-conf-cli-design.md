...
**행동 규칙이다. 조건과 그때 무엇을 하는지만 적는다.**

| | 조건 | **무엇을 한다** |
|---|---|---|
| **(가)** | `process.stdin.isTTY && process.stdout.isTTY` 가 참이 아니다 | **`conf.json` 을 만들지 않고 종료한다** (exit 1) |
| **(나)** | 워커다 (`cluster.isPrimary` 가 거짓) | **묻지 않는다.** `conf.json` 이 없으면 오류로 종료 |
| **(나)** | 마스터인데 파일 생성에 실패했거나 사용자가 취소했다 | **`require('./app')` 을 하지 않는다** — cluster 를 만들지 않고 종료 |
...
**지금 동작(기본값으로 파일을 만들고 진행)을 바꾸는 것이다.** 만들어 두면 다음 기동에
마법사가 안 돌고(파일이 있으니), `dbpass` 가 소스에 박힌 값이라 DB 연결에서 실패한다 —
원인이 두 단계 멀어진다.

`stdin` 과 `stdout` 을 **둘 다** 본다. `stdin` 만 보면 `npm start > log` 처럼 출력만
리다이렉트한 경우를 놓친다 — 입력은 받는데 **무엇을 묻는지가 파일로 가서** 사람은 빈 화면
앞에 앉는다.

**Ctrl-D(EOF)도 취소로 본다.** "기본값 선택"으로 읽으면 비대화형에서 조용히 잘못된 파일이
생긴다 — §4.3 의 관문이 EOF 를 거부로 정한 것과 같은 이유다.

이것을 안 지키면 프로세스가 **살아 있고 포트는 닫혀 있는데 pm2 는 `online` 으로 보는**
상태가 된다 — §11.6 의 좀비와 같은 모양이고, 마법사를 넣으면서 없애려던 것을 새로 만드는 셈이다.

**(나) — 실패하면 fork 에 도달하지 않는다.**

`cluster.fork()` 가 `mobius.js` 를 처음부터 다시 실행하므로 워커도 이 코드를 지난다.
그리고 **워커는 부모의 stdio 를 상속하므로 마스터가 TTY 면 워커도 TTY 다** — (가)로는 못 막는다.

```js
if (!cluster.isPrimary) {
    // 워커에서 conf.json 이 없는 것은 마법사가 아니라 오류다.
    // 마스터가 이미 만들었어야 한다.
    return callback(new Error('워커에서 conf.json 을 찾지 못했다'));
}
```

마스터가 마법사를 돌렸는데 **파일 생성에 실패하거나 사용자가 취소하면 `require('./app')`
을 하지 않는다.** cluster 자체가 안 생기므로 프롬프트가 16개 뜨는 상황이 **원천적으로
없어진다** — fork 에 도달하지 않으니까.

**부분 저장을 하지 않는다.** `conf_store` 의 원자적 쓰기(임시 파일 + `rename`)를 쓰면
반쪽 파일이 원천적으로 안 생긴다.
...
conf_load(function (err, applied) {
    if (err) { console.error('[설정] ' + err.message); process.exit(1); return; }
    boot_record.write(applied);
    require('./app');            // 실패하면 여기에 도달하지 않는다 — (나)
});
```

`require('./app')` 이 한 tick 밀리는 것뿐이고 새 의존성이 없다(`readline` 은 내장).
워커에서는 `conf.json` 이 있으므로 동기적으로 끝나고 콜백을 즉시 부른다.

**대신 `mobius.js` 의 줄이 밀린다.** 그 파일을 **문자열로 검사하는** 시험이 여럿이라
(`conf-schema` 의 기본값 대조, `usesqlite-single-reader` 의 **줄 번호** 허용 목록)
같이 손봐야 한다 — §8.0.
...
**⑰ 비대화형에서 파일을 만들지 않고 종료한다** — 파이프로 실행했을 때 (1) 매달리지 않고
(2) **`conf.json` 이 안 생기고** (3) exit 1 인지. 셋 다 본다 — 매달리면 pm2 가 `online`
으로 보고, 파일이 생기면 다음 기동에 마법사가 안 돈다

**⑱ 마법사가 만든 `conf.json` 이 유효하다** — `db` 선택지가 `backends()` 에서 오는지,
SQLite 를 고르면 `dbpass` 를 안 묻는지, 만든 파일을 `conf_load` 가 그대로 읽는지.
**취소하면 파일이 안 생기는지도** — 부분 저장 금지
