...
`--superuser`)이 값을 바꾼 뒤 다시 봉인한다. 별도 `--seal` 명령은 두지 않는다. 봉인은 언제나 재입력
경로(`npm run setup -- --superuser`/`--dbpass`)의 부수 효과이고, 그 프롬프트의 **Enter 는 값을 그대로
두고 봉인만 새로 만든다**(사용자 결정 2026-09-05). 손으로 고친 뒤 Enter 로 봉인할 수 있다는 점은
받아들인다 — 봉인은 비밀 유지가 아니라 '도구를 거치게 하는 것' 이고, 같은 계정은 어차피 key 로
HMAC 을 계산할 수 있다(한계 절). `npm run conf -- set` 은 비밀을 못 건드리므로 봉인을 깨지 않는다.
