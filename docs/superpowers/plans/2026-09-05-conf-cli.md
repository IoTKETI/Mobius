...
- [ ] 서버에서 새 코어로 재기동하기 **전에** `npm run setup -- --superuser`를 한 번 쳐서 `conf.seal.json` 을 만든다 — **Enter 한 번이면 된다**(값은 그대로 두고 봉인만 만든다·이미 있으면 다시 만든다). `--dbpass` 도 같은 규칙이다. 봉인이 없으면 재기동이 거부된다(exit 1, 안내 출력).
