...
function arm(req, label, ms) {
    ...
    // ── 이것은 **총 소요시간 상한이 아니다** ────────────────────────────
    //
    // req.setTimeout 은 소켓이 **조용한 구간**을 잰다. 바이트가 하나라도
    // 오면 타이머가 처음부터 다시 시작한다. 그래서 상대가 한도보다 짧은
    // 간격으로 조금씩 흘려보내면 영원히 안 끊긴다. 실측했다:
    //
    //     한도 400ms, 250ms 마다 1바이트   -> 3,016ms 까지 안 끊김 (계속 흐름)
    //     한도 400ms, 완전 침묵            -> 413ms 에 끊김
    //
    // 위 주석의 "응답 바디를 안 주면" 은 **완전 침묵**만 가리킨다. 느린
    // 드립은 이 방어선에 안 걸린다.
    //
    // 총 상한을 걸지 않은 것은 의도다 — 정상적으로 느린 상대(큰 응답을
    // 천천히 보내는 쪽)를 끊게 되므로, 근거 없이 넣을 값이 아니다. 필요해지면
    // 배포 로그의 [outbound] 항목과 응답 크기 분포를 먼저 봐야 한다.
