'use strict';
// 코어 전체를 훑는 부재 불변식 (요청 흐름 남은 일 §7.3).
//
// 이 저장소의 소스 감시 시험 대부분은 파일 목록을 손으로 적는다. 그 시험들은 "그 자리"
// 를 정확히 지키지만, 같은 코드가 목록 밖 파일로 옮겨 가면 조용히 통과한다 — 예컨대
// cache_resource_url 이 mobius/cnt.js 에서 되살아나면 test/no-resource-cache 는 모른다.
//
// 여기서는 그 시험들이 지키는 것 중 **어느 파일에서든 있어서는 안 되는** 것만 골라,
// 추적되는 실행 파일 전체(test/lib/sources.js)를 훑는다. 원래 시험은 그대로 둔다 —
// 자리와 근거는 거기에 있고, 여기는 목록 밖을 막는 그물이다. 새 불변식은 아래 표에
// 한 줄 더한다. `allow` 는 그 패턴이 정당하게 있는 파일이다 — 다른 파일에 나타나면
// 검토 대상이다.
//
// 변이로 증명한다(scratchpad mutate-invariants): 패턴을 원래 목록 **밖** 파일에 심으면
// 원래 시험은 통과하고 이 시험이 잡는다.
...
test('불변식 표 자체의 건강 — 범위마다 파일이 있고, allow 는 실재하는 파일이다', function () {
    ...
    // allow 에 적은 파일에는 그 패턴이 실제로 있어야 한다 — 없으면 allow 가 낡은 것이다
    INVARIANTS.forEach((inv) => (inv.allow || []).forEach((f) => {
