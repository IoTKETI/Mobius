'use strict';
// Contract of the conf CLI. The logic is tools/conf_cli.js, the entry point tools/mobius-conf.js.
...
// deps that behave like the real ones; record, liveness, port and pm2 are set by the test.
function deps(opts) {
    opts = opts || {};
...
test('L1 판정 — 마스터 pid 가 살아 있으면 포트가 닫혀 있어도 모름이 아니다', function () {
    ...
    // Absent from the file, the default is the file value
...
test('L2 비교는 정규화한다 — 객체가 든 배열, 숫자/문자열', function () {
    ...
    // A file with a number for a string-typed key is caught by checkValue first: 'invalid', not 'pending restart'
...
// Uses advanced keys, so --all (visibility is tested elsewhere); otherwise the advanced-key gate catches first and changes the reason under test
...
// Uses advanced keys, so --all (visibility is tested elsewhere)
...
test('L6 관문 — EOF 는 거부. 키 이름을 정확히 치면 통과', function (t, done) {
    ...
    setImmediate(() => eof.end());   // Ctrl-D without typing anything
...
test('L7 pm2 앱은 이름이 아니라 pid 로 고른다', function (t, done) {
    const rec = ALIVE_REC({ csebaseport: '7579' });
    const list = [
        { pid: 1, name: 'Mobius', pm2_env: { status: 'online', restart_time: 99 } },     // same name, different process
    ...
    cli.runSet('dbConnectionLimit', '30', d, function (err, r) {
        ...
        cli.runUnset('dbConnectionLimit', d, function (err2, r2) {
            ...
            cli.runSet('dbConnectionLimit', '30', a, function (err3, r3) {
                assert.strictEqual(r3.ok, true, r3.lines.join(' '));
                assert.strictEqual(readConf(d.file).dbConnectionLimit, 30);
                // User keys work without --all; a gate key is refused on a non-TTY, but the reason must not be 'advanced key'
...
test('B3 진입점이 sealStatus 를 conf_seal.verify 로 배선한다 — 소스 검사', function () {
    // The default deps.sealStatus stub is { ok: true } (deps() helper), so every list test passes even with the wiring broken; a source pin like the --all wiring pin is added.
    const src = fs.readFileSync(path.join(ROOT, 'tools', 'mobius-conf.js'), 'utf8');
...
// The order the user keys are asked = the table's group declaration order (superUser is a secret and excluded; db, csebaseport, then cseBase, cseId, spId). The answer array follows the order renderList shows.
test('E1/E2 edit — Enter 는 그대로, 바꾼 키만 한 번에 쓴다; 파일에 없던 키를 Enter 로 지나면 안 생긴다', function (t, done) {
    ...
    // store.update calls are counted directly. 'Written in one go' cannot be measured from the file content alone, because several writes could produce the same result.
    let updateCalls = 0;
    const origUpdate = d.store.update.bind(d.store);
    d.store.update = function (patch) { updateCalls++; return origUpdate(patch); };
    scriptEdit(d, ['', '7580', '', '', '', 'csebaseport']);   // db kept, csebaseport changed (gate -> key name at the end), cseBase, cseId, spId kept
    cli.runEdit(d, function (err, r) {
        ...
        // An unchanged gate key (cseBase) is not asked for confirmation. An implementation asking every gate key could pass this test by accident (gate order), so this assertion measures 'only when changed'.
...
test('E4 관문 키는 바꿨을 때만 확인을 받고, 거부하면 그 키만 빠진다', function (t, done) {
    const d = deps({ conf: { db: 'mysql' }, isTTY: true, stdin: new PassThrough() });
    scriptEdit(d, ['sqlite', '', 'Vita', '', '', 'nope']);   // db (edit) changed, cseBase (gate) changed -> refused at the gate with 'nope'
    cli.runEdit(d, function (err, r) {
        assert.strictEqual(r.ok, true, r.lines.join(' '));
        const saved = readConf(d.file);
...
test('D6 관문 키 둘을 한 번에 바꾸면 승인한 것만 써진다', function (t, done) {
    const d = deps({ conf: { db: 'mysql' }, isTTY: true, stdin: new PassThrough() });
    // db and csebaseport kept, cseBase and cseId (both gates) changed, spId kept -> gate confirmation cseBase first (approved), then cseId (refused).
...
test('E5 비-TTY 와 EOF 는 아무것도 쓰지 않는다', function (t, done) {
    ...
    cli.runEdit(d1, function (err, r) {
        ...
        scriptEdit(d2, ['sqlite']);   // EOF after one answer
        ...
        cli.runEdit(d2, function (err2, r2) {
            ...
            // Ctrl-C/EOF at the gate confirmation prompt (not at a question) must drop every edit-grade key collected so far (db) as well; the file must not change against the intent of a user who cancelled at the gate warning.
            const d3 = deps({ conf: { db: 'mysql' }, isTTY: true, stdin: new PassThrough() });
            scriptEdit(d3, ['sqlite', '', 'Vita', '', '']);   // db (edit) and cseBase (gate) changed, then EOF at the gate prompt
...
test('E6 --all 이면 고급 키도 묻는다; 기본은 사용자 키만이고 비밀은 안 묻는다', function (t, done) {
    ...
    scriptEdit(d, []);   // EOF at the first prompt
