/**
 * Miscellaneous invariants (get_ri_list_sri is in test/ri-sri-batch.test.js):
 *
 *   - access_value literals -> security.ACOP constants. The values are the oneM2M acop bits.
 *   - removed code: global.useobserver, ty '33' in security.js, the mgo branch of check_allowed_app_ids, the sgnManPort/hitManPort conf keys (read but never listened on)
 */
'use strict';
...
test('security.ACOP 는 oneM2M acop 비트이고 문자열이다', () => {
    ...
    // acop_allows compares with string &; changing to numbers requires changing that site too.
...
test('CORS 는 cors() 한 곳이고 Expose-Headers 에 X-M2M-RSC 가 있다', () => {
    // A manual CORS middleware would duplicate cors(). Browser clients need Expose-Headers to read X-M2M-RSC from the response.
    const src = code('app.js');
    ...
    // Kept as a string: an array would be joined with ',' by cors and change the bytes of the header value.
