'use strict';
/**
 * Pins what editing conf.json must preserve.
 *
 * The file is edited by several sessions and people at once, and every Mobius worker reads it at boot, so 'preserve unknown keys' and 'write atomically' are consistency requirements, not conveniences.
 */
...
test('exposed:false 인 키는 전부 고칠 수 없다 — 목록을 표에서 전수로 뽑는다', function () {
    // Hidden keys are taken from the table instead of a hand-written list; a hand-written entry that had left the table was refused as 'unknown key' and the check ran idle.
...
test('정수 키는 범위를 지킨다', function () {
    ...
    // The value the screen draws an integer field for; the core blocks it with the integer flag.
...
test('outboundTimeoutMs 는 끄거나(0) 3초 이상이어야 한다', function () {
    // The response wait limit of notification sending and remote CSE forwarding. Too short, normal notifications start being recorded as failures; worse than off.
...
test('db 의 유효값은 코어가 어댑터에서 실시간으로 준다', function () {
    ...
    // No hard-coded list; adding an adapter must show up on the screen by itself. Only compared with what is attached now.
...
test('콘솔 자신의 conf 키를 "모르는 키" 로 오해하지 않는다', function () {
    // One conf.json holds Mobius's keys and the console's. The core schema knows only what mobius.js reads and not adminPort and the like; unless supplied, the screen shows them as 'keys another session added'.
    const file = tempConf({
        acpAudit: 'on',
...
test('콘솔 키는 고칠 수 있다 — 설정은 이제 CLI 의 일이다', function () {
    // The rule that kept the web from editing its own adminPort lost its meaning once the web stopped editing configuration; the CLI is used by the person on SSH.
    ...
    // The console's secret keys are still refused
...
test('파일에 없는 키는 기본값을 쓴다고 구분해서 말한다', function () {
    const file = tempConf({ dbpass: 'x' });   // no policy key at all
    const v = store(file).view();
...
test('적용 시점을 키마다 말한다 — 저장하면 언제 반영되는가', function () {
    ...
    // Read from the global on every request: changing the value takes effect at once
    ...
    // Cached by the module: configure() must be called again
...
test('쓴 결과는 언제나 온전한 JSON 이다', function () {
    ...
    for (let i = 0; i < 20; i++) {
        ...
        const parsed = readConf(file);   // fails if it throws
...
test('모든 항목이 분류를 갖는다 — 화면에서 조용히 빠지지 않게', function () {
    // The group is decided by the core (describe().group); the screen decides only the order. An empty group goes to 'other', a sign that the core gave no classification; caught here early.
    const file = tempConf({});
...
test('reload 키는 무엇을 다시 불러야 하는지 알려 준다', function () {
    // The core's describe() carries reloadWith; without it the screen could only say 'applies without restart' and not what is needed.
    const file = tempConf({});
    const items = store(file).view().items;
...
// --- APIs used by the CLI and the wizard ---
...
test('create — conf.json 은 만들어졌지만 봉인이 실패하면 사유를 가른다', function () {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'confstore-'));
    const file = path.join(dir, 'conf.json');
    // A directory pre-created where conf.seal.json goes makes writeAtomic's rename fail; conf.json is already written by then, so this must not be lumped into 'could not create'.
...
test('W3 setSecret 은 dbpass·superUser 만, 파일이 있을 때만', function () {
    ...
    // superUser takes the same path; a key absent from the file is added
...
// --- Re-entry Enter = keep the value and seal ---
...
test('reseal — 봉인이 실패하면 create 와 같은 모양의 사유로 던진다', function () {
    ...
    // A directory pre-created where conf.seal.json goes makes writeAtomic's rename fail.
    fs.mkdirSync(seal.sealPath(file));
    const s = store(file);
...
test('conf_schema.checkValue 는 관문 없이 타입·유효값만 본다', function () {
    ...
    // groups() is in declaration order
