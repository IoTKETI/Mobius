'use strict';
// Request address resolution: a structured address is looked up by ri (the primary key) alone.
//
// When the first segment is the CSE base name there is nothing to look up by sri, so only ri is queried. Unstructured addresses (`/~/<cseid>/<id>`, `/<id>`) look at both columns.
...
// Intercepts the adapter and looks only at the generated SQL (as tapCtx in test/migrate.test.js)
const seen = [];
...
test('비구조 주소 — 지금처럼 ri 와 sri 둘 다 본다', async () => {
    ...
    // knex wraps the object condition of orWhere in parentheses
