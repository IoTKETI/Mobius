'use strict';
/**
 * Resolves ID-form entries of a subscription's nu with three queries (sri unfolding, lookup, type table) instead of two sequential queries per nu. Decisions and log wording are unchanged.
 *
 * The deployment has no ID-form nu, so this pins structural correctness (2M round trips per subscription down to 3) and the rule 'unresolved nu entries are dropped and the rest continue'.
 */
