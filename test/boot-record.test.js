'use strict';
// Contract of the boot record (log/mobius-boot.jsonl). Writes a real file, in a temporary directory.
...
test('C7 새 비밀 키가 생겨도 자동으로 빠진다 — 표를 훑지 목록을 들지 않는다', function () {
    ...
    // The real table with one key added as a secret; everything else is delegated to the real one.
...
test('C7 log/ 를 만들 수도 쓸 수도 없을 때 던지지 않는다 — 기동을 막지 않는다', function () {
    // A file where the directory should be: both mkdir and open fail.
...
test('C8 상한 — 끝에 capped 한 줄만 덧붙고 마스터 줄이 남는다. 이미 있으면 더 안 쓴다', function () {
    ...
    // Fills up to just below the cap (the master line included)
    for (let i = 1; i < cap; i++) {
    ...
    // The worker that reaches the cap appends capped instead of its own line
    ...
    // Once capped is present nobody writes any more
