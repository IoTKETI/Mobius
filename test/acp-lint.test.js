'use strict';
// Checks ACPs and acpi that are already stored.
//
// The guardrails block only new writes; invalid stored values keep producing HTTP 500 or silent refusals. The linter fixes nothing; it only returns a list.
...
// An adapter that returns the planted select results in order.
...
test('acp 행이 없는 ACP 를 잡는다 — 잠금이 조용히 풀린다', function (t, done) {
    // Query 1: select_acp_list, query 2: select_acp_in
