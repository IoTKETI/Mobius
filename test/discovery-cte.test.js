...
function tap(backend, opts) {
    ...
    adapter.execute = function (conn, sql, bindings, cb) {
        ...
        // limit 뿐 아니라 offset 도 흉내 낸다. 예전 경로(ofst / la)는 DB 가
        // offset 을 건너뛰므로, 여기서 무시하면 2페이지가 1페이지와 같아진다.
...
test('ofst 가 있으면 배치를 쓰지 않고 예전 한 문장으로 간다', function (t, done) {
    // **왜 예전 경로인가.** 전역 오프셋은 SQL 이 DB 안에서 건너뛴다. 배치로
    // 나누면 그럴 수 없어 JS 가 앞을 버려야 하고, 그러려면 배치마다
    ...
    run(h, { ty: '3', lim: 20, ofst: 6 }, guard(done, function (code, ris, seen) {
        assert.strictEqual(seen.length, 1,
            'ofst 요청이 문장을 여러 개 던졌다: ' + seen.length);
        const sql = oneShotStmt(seen).sql;
        assert.match(sql, /offset 6\b/, 'offset 을 DB 가 건너뛰지 않는다: ' + sql);
        assert.match(sql, /limit 20 offset 6$/,
            '한도가 (오프셋 + 한도) 로 부풀었다 — 버릴 행까지 실어 온다: ' + sql);
        assert.ok(!isChild(seen[0]), '자식 배치 질의가 나갔다');
        done();
    }));
});

test('부모가 가진 자식보다 큰 ofst 도 예전 그대로 동작한다', function (t, done) {
    const h = tap('mysql');
    run(h, { ty: '3', lim: 20, ofst: 10 }, guard(done, function (code, ris, seen) {
        assert.strictEqual(code, '200');
        assert.strictEqual(seen.length, 1);
        assert.match(oneShotStmt(seen).sql, /limit 20 offset 10$/);
        done();
    }));
});

test('페이지가 겹치지 않는다', function (t, done) {
    // 1페이지는 ofst 가 없으므로 배치 경로, 2페이지는 ofst 가 있어 예전 경로다.
    // **두 경로가 같은 순서를 봐야** 페이지가 겹치거나 빠지지 않는다.
    const a = tap('mysql');
...
test('골격이 커도 ofst 가 있으면 배치로 가지 않는다', function (t, done) {
    const h = tap('mysql', { skeleton: bigSkeleton(9001) });
    run(h, { ty: '3', lim: 3, ofst: 2 }, guard(done, function (code, ris, seen) {
        assert.strictEqual(seen.length, 1,
            '부모 9,001개에 ofst 가 있는데 배치로 갔다: ' + seen.length);
        assert.ok(isOneShot(seen[0]), '단일 문장이 아니다');
        assert.match(seen[0].sql, /limit 3 offset 2$/,
            '한도가 (오프셋 + 한도) 로 부풀었다: ' + seen[0].sql);
        done();
    }));
});
