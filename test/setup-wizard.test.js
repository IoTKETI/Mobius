'use strict';
// First-run wizard. Prompts are in mobius/setup_prompt.js, core wiring in conf_load, entry point tools/setup.js.
//
// The TTY stand-in behaves like the real thing: readline opened with terminal:true calls input.setRawMode(true) and resets it to false on close; isRaw toggles as on a real TTY.
// This file also selects the sqlite backend, which freezes the key table, so it is kept separate from other tests.
...
// Supplies one answer per prompt. EOF once the answers run out.
...
// The table and the facade freeze on global.usedb at require time (mergeBackendConf, pick cache). This file alternates backends, so both caches are cleared at each selection; conf_load's first_run passes the same require, so they are cleared before the conf_load tests as well (fresh).
function fresh() {
...
test('W2 답을 다 받으면 일곱 키 이하의 answers — sqlite 면 dbpass 를 묻지 않는다', function (t, done) {
    ...
    const t1 = io();
    // Order: DB, (dbpass), CSE name, CSE ID, SP-ID, superuser Origin, HTTP port
    ...
    setup_prompt.run({ backends, onBackend, io: { stdin: t1.stdin, stdout: t1.stdout } }, function (err, answers) {
        ...
        // Prompt order: superuser after SP-ID, then the port.
        const seen = t1.seen();
...
test('W1 취소(EOF) — CANCELLED, raw 모드가 돌아온다', function (t, done) {
    const backends = db.backends();
    const t1 = io();
    script(t1, [String(backends.indexOf('sqlite') + 1)]);   // EOF after one answer
...
test('C10/S6 mobius.js 는 워커의 NO_CONF·BAD_SEAL 을 전용 코드로 내고, app.js 마스터가 그것을 보면 재포크하지 않는다', function () {
    ...
    // Worker-only exit codes come from the table (WORKER_EXIT): NO_CONF(13), BAD_SEAL(14). A list of literal comparisons diverges at the third code.
