/**
 * `mobius/fanout.js` sends member requests in bounded parallel. The test runs a real HTTP member server so the server, not the test, determines how many requests were in flight at once.
 *
 * What breaks this test:
 *   - bound set to 1 (serial)               -> max concurrent requests 1, duration N times longer
 *   - bound removed                          -> max concurrent requests N
 *   - agr filled in completion order         -> key order differs from mid order
 *   - failed member not excluded / whole fan-out failed -> other members vanish or no callback
 */
'use strict';
...
// One member server serves several member paths. Delay and body are configurable per path, and the peak number of concurrently open requests is counted.
function memberServer(rule) {
    ...
    s.server = http.createServer((req, res) => {
        ...
        req.on('end', () => {
            ...
            if (r.silent) { s.pending.push(res); return; }      // never answers
            setTimeout(() => {
                s.inflight--;
...
test('멤버 12개가 각 300ms 걸릴 때 동시에 8개까지만 나가고 전부 집계된다', async () => {
    ...
    try {
        ...
        // Serial would take 12 x 300 = 3,600ms; with a bound of 8 in parallel it is two rounds, about 600ms.
...
test('첫 멤버가 가장 늦게 답해도 agr 의 키 순서는 mid 순서다 — 응답 본문이 직렬일 때와 같다', async () => {
    ...
    try {
        ...
        // The result object keeps the shape of the former check_body: fr, rsc, rqi, pc.
...
// fopt.js side, checked by source: requiring resource.js opens an MQTT client.
function live(rel) {
...
test('fopt.js 는 멤버마다 조회하지 않는다 — 원값과 접은 값을 합쳐 한 번에 풀고 옛 우선순위로 고른다', () => {
    ...
    // In lookup ri is the structured path and sri the short id. Whether the raw or the folded value is the sri is unknown in advance, so both are queried; querying only one differs from the old code for some inputs (e.g. an AE-ID starting with 'Mobius' written verbatim in mid).
    ...
    // grp.mid is folded on a copy; make_internal_ri modifies in place.
