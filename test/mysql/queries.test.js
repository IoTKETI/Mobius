'use strict';
// 핵심 질의를 실제 MySQL 에서 실행하고 EXPLAIN 으로 인덱스 사용을 본다 (§7.1).
//
// 스텁 시험은 SQL 문장의 모양까지만 본다. 여기서는 빈 시험 DB 에 빌더로 트리를 심고
// (CSEBase → AE → 컨테이너 넷 × CIN 400 + 3단 중첩 컨테이너 + 구독 + ACP), 요청 경로의
// 질의를 그대로 돌린 뒤 **기록된 문장을 EXPLAIN** 한다. 2026-09-06 의 콜레이션 캐스트
// (재귀 조인이 인덱스를 잃어 30초 타임아웃)는 여기 골격 EXPLAIN 이 잡았을 결함이다.
//
// 표는 작다(수천 행). 규모에서만 드러나는 것(통계로 뒤집히는 계획, 시간)은 배포 절차의
// 운영 DB EXPLAIN 이 본다 — CLAUDE.md 배포 절차 참고.
...
// 기록된 문장 중 re 에 맞는 마지막 것을 EXPLAIN 한다
const notExplain = (s) => !/^\s*explain\b/i.test(s.sql);   // 우리가 낸 EXPLAIN 자체는 다시 보지 않는다
...
// 검색이 남긴 lookup 질의 전부를 EXPLAIN 한다 — 어느 것에도 lookup 풀스캔이 없어야 한다
...
test('discovery 골격(재귀 CTE) — AE 아래 컨테이너 여섯, 재귀항이 (pi, not_cin) 을 타고 풀스캔이 없다', async () => {
    // CSEBase 가 대상이면 골격을 만들지 않는다(한정할 것이 없다). AE 아래로 묻는다.
    tap.seen.length = 0;
    const found = {};
...
test('ACP 조회 둘 — 부모 재귀(select_acp_cnt)와 IN(select_acp_in)', async () => {
    // 실제 호출 모양: 요청 URL 을 '/' 로 쪼갠 배열, loop 0 부터 부모 → 조부모 … AE 까지
