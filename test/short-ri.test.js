'use strict';
// 짧은 리소스 id(sri) 생성기 (ri/sri 설계 메모 §3 A1).
//
// 옛 생성기는 `ty-YYYYMMDDHHmmssSSS` 뒤에 난수 세 자리를 붙였다. 워커 24개가 각자
// 만드니 같은 ms 에 두 건이 나면 1/1000 로 겹친다 — 배포 실측으로 8월 한 달에
// 같은 ms 가 16,887번, 실제 중복 16개. 인덱스가 비유일이라 DB 도 막지 못했다.
//
// 새 생성기는 한 호스트 안에서 **구성적으로** 유일하다: pid 는 살아 있는 동안
// 유일하고, seq 는 프로세스 안에서 같은 ms 에 증가한다. 여기서는 그 성질을
// 실제 폭주(한 프로세스 20만 건 · 프로세스 넷 동시)로 본다.
...
test('모양 — <접두><UTC 17자리><pid·seq base36>, 45자 안, URL 안전 문자만', () => {
    ...
    // 접두는 호출부가 준다 — 타입 번호는 대시까지, AE 는 S/C 만
...
test('같은 ms 안에서는 seq 가 늘고, 시계가 뒤로 가도 반복하지 않는다', () => {
    // 시계를 고정하고 두 번, 뒤로 돌리고 한 번 — 셋 다 달라야 한다.
    // 생성기는 프로세스 안에서 단조라(앞 시험이 지금 시각을 썼다) 고정 시각은 미래로 잡는다.
...
test('프로세스 넷이 동시에 3만 건씩 — 서로 겹치지 않는다', async () => {
    ...
    // 프로세스마다 pid 조각이 다르다 — 시각 뒤 다섯 자리(이상)
