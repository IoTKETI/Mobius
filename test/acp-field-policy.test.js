/**
 * The pv/pvs policy lives in one place (security.js).
 *
 * security.js exports the tables (FIELD, INHERITS_ACPI) and field_of, and check() and the simulator read only those. The check() cases below are a characterisation of the decisions as they are.
 */
'use strict';
...
test('FIELD — pv 는 remoteaddress 를 보고 acr 없는 규칙에서 생성자로 끝내며, pvs 는 둘 다 아니다', () => {
    // The literals of the former two wrappers as they were. Changing a value changes decisions; that is a separate matter.
...
test('mobius/acp_eval.js 는 없고, 평가기 정의는 security.js 에만 있다', () => {
    // The evaluator stays inside security.js; a separate acp_eval.js must not exist.
...
// ── check() end-to-end characterisation with a stubbed DB ──
...
// The sri->ri substitution step is not part of the ACP path. The stub stays but counts its calls; the test below catches a revival. The acpi validation of fopt and resource still uses this function.
calls.ri_list_sri = 0;
global.get_ri_list_sri = function (request, response, list, out, count, cb) { calls.ri_list_sri++; list.forEach((v, i) => { out[i] = v; }); cb('200'); };
global.make_internal_ri = function () { /* test input is already in internal form */ };
...
test('ACP 검사는 두 질의다 — 재귀(select_acp_cnt)와 IN(select_acp_in). sri→ri 치환 단은 없다 (§5.3)', async () => {
    // Stored acpi is already in ri form (validate_acpi normalises it and make_internal_ri folds the notation), so acpi goes to select_acp_in as is, without get_ri_list_sri.
