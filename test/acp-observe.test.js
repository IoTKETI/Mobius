'use strict';
// Denial observation and observe mode.
//
// security.check is the hot path of every request: an observer that throws kills the request, and an unthrottled log lets one wrong ACP fill the disk on every worker.
...
test('관찰 모드는 기본 정책 거부를 뒤집지 않는다', function () {
    // Most real denials are default_policy. Flipping every denial regardless of reason would turn observe mode into opening every resource to UPDATE/DELETE by any originator, and what is deleted in that window does not come back.
...
/*
 * ── pvs: access to the ACP itself ──
 *
 * security.check decides by pvs (selfPrivileges) when the target is an ACP (ty=1): not 'may this resource be read or written' but 'may this access policy itself be changed or deleted'.
 *
 * Observe mode assumes 'watch without blocking, reversibly'. Flipping pvs breaks that: during the window anyone authenticated could change or delete the ACP body, and turning the mode off does not undo it; every resource referencing the ACP changes permissions with it.
 */
...
test('pvs 는 path 로 와도 막는다 — security.js 가 두 이름을 쓴다', function () {
    // evaluate_acp_rows records the field in trace.field and the ty=1 branch in trace.path; both must be checked.
    reset({ mode: 'observe' });
    quiet(function () {
        assert.strictEqual(
            observe.record_decision(req('AE-X', '/Mobius/acp1'), '0',
