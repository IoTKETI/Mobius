...
/** Waits until the job ends; state leaves running only. */
...
test('한 번에 한 작업만 돈다', async function () {
    ...
    await settled(first);
    // After it ends a new job can start.
...
test('취소하면 남은 대상을 시작하지 않는다', async function () {
    ...
    // Items already started run to completion; cutting mid-way would leave it unknown whether the server deleted.
...
test('동기로 끝나는 worker 도 스택을 넘기지 않는다', async function () {
    // A worker that calls back synchronously (e.g. an immediate skip in the preflight) would grow the stack by the number of targets if pumped recursively.
    const job = jobs.start({
        kind: 'test', title: 't',
        concurrency: 1,
