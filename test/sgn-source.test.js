'use strict';
/** The source of notification routing is the sub table. sgn.check reads `sub where pi = <ri of the resource the subscription is attached to>` once per write, on its own connection. */
...
// Inspects the SQL actually emitted, the same way as converted-queries' tapAdapter.
function tapSql() {
...
test('select_resources_in — lookup 한 번(ri in … or sri in …) 뒤 타입별 테이블 한 번씩, merge', (t, done) => {
    const { sql_action, calls } = tapSql();
    calls.rows = [
        // ty 29 (timeSeries) is a removed type and absent from typeRsrc; '99' exists as rsp and is not unsupported.
...
// Which rows are sent to (sub_source).
...
test('rows_for — DB 오류면 로그 한 줄과 빈 목록 (알림은 fire-and-forget)', async () => {
    ...
    // A callback that never arrives would only hang the test, not fail it, so a timeout is applied. Omitting the callback on the error branch also skips release and leaks the connection.
