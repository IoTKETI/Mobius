'use strict';
// Migration runner. Code is overwritten on deploy; schema is not, so schema changes are applied through migrations/ with a history table.
...
// A later migration may assume the earlier one, so the run must stop at the first failure.
...
// With the default 60-second driver timeout the driver drops the connection while MySQL continues the DDL; the runner reports failure and records nothing while the index is still created.
...
// MySQL has no CREATE INDEX IF NOT EXISTS. Re-running after the situation above (timeout, server completed) must not fail with 'Duplicate key name'.
...
// The facade must pass opts through to the adapter for the two tests above to hold.
