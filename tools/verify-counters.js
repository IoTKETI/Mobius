'use strict';
// Verifies every case of counter / stateTag maintenance against a live server.
//   node mobius.js sqlite &        # or mysql
//   node tools/verify-counters.js
//
// Each case manipulates resources over HTTP and then checks that the read cni/cbs/st match the expectation. A mismatch between the stored values (get_cni_count) and the actual data shows up here.
...
async function call(method, path, opts) {
    ...
    let body = text;
    try { body = JSON.parse(text); } catch (e) { /* kept as is */ }
...
// Reads a container's cni/cbs/st.
...
// Parent st checks must use a container as the parent: an AE response has no st (oneM2M defines no stateTag for AE, so it is omitted from the response although stored in the DB).
...
// Waits for asynchronous work (sweeps, background updates) to settle.
function settle(ms) { return new Promise(function (r) { setTimeout(r, ms || 2500); }); }

async function main() {
    console.log('대상: ' + BASE + '\n');

    // Start from a clean state (repeatable)
    ...
    // --- Case 1: creating CINs raises cni/cbs ----------------------------------
    console.log('[1] CIN 생성 -> cni/cbs 증가');
    await makeCnt('c1');
    // No delay between creations: rn carries a per-worker monotonic sequence (mobius/rid.js) and the la sort uses ri after ct, so the order is exact within the same second.
    ...
    // --- Case 2: deleting a CIN lowers cni/cbs ---------------------------------
    ...
    // --- Case 3: retention limit exceeded -> oldest purged, st raised ----------
    ...
    // --- Case 4: creating a child CNT raises the parent's st -------------------
    // The parent must be a container so st is visible in the response (see readStAt).
    ...
    // --- Case 5: deleting a non-CIN child raises the parent's st ---------------
    ...
    // --- Case 6: SUB creation/deletion raises the parent's st ------------------
    ...
    // --- Case 7: stored values match the actual data ---------------------------
    ...
    // --- Case 8: lowering mni takes effect at once (latest DB value) -----------
