...
// A CIN's rn is its creation time and differs per run, so it is replaced by its ordinal within the parent; rn has a fixed width, so lexical order is creation order (mobius/rid.js).
function normalizeCin(snap) {
    for (const k of Object.keys(snap)) {
        ...
        // la / ol are compared by content instead of rn.
