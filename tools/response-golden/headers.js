'use strict';
/*
 * Records the headers and body shape of responses; run before and after changing responder and compare.
 *
 *   node tools/response-golden/headers.js <port> <output file>
 *   node tools/response-golden/headers.js --diff <before> <after>
 *
 * The golden harness in the same directory (tap.js -> collect.js -> compare.js) records only fn / status / rsc / dbg / method, not headers.
 *
 * Values that change per run are removed or replaced by placeholders, so a diff shows nothing when the code is unchanged:
 *   date / content-length / etag / connection / x-m2m-ri  -> removed
 *   originator name, CIN ri                                -> <ORIGIN>, <CIN-ID>
 * Of the body only the set of top-level keys is recorded; values contain times and ids.
 */
...
/* A header value of this shape means something was turned into a string wrongly (e.g. `X-M2M-RSC: [object Object]` from String() on a catalogue object). A diff cannot catch it when before and after are equally wrong, so the values themselves are checked as well. */
...
function collect(PORT, OUT) {
    ...
    function (a) {
        ...
        var steps = [
            ...
            // The producer names the shape directly, so every shape is exercised once: rcn=2 (a single uri) and fu=2&rcn=4/5/6 (grouped, root m2m:rsp). fu=1 is uril and separate.
            ...
            // What is answered without Accept.
            //
            // Always json: neither Accept nor the request Content-Type changes the response format; responder.apply_headers fixes application/json in one place. The three lines stay so a divergence in any of the three paths (response_result, search_result, response_rcn3_result) shows up here.
            ...
            // json-only gate. The 'xml body' case above already covers one, but the gate must tell three apart: two rejected MIME types and one normal request whose parameters merely contain the letters xml. If the last one becomes 400, the gate has regressed to a substring match.
