// Gap check: (A) creates, reads, updates and discovers the eight fcnt/hd_* moduleclasses on a real DB and receives the subscription notification prefix (nev.rep key); (B) the AE notify path (POST body sgn relayed to the AE's poa).
// A different AE name per run. Values are dropped; only (status, rsc, root key, key structure and types) are kept.
//
...
// GAP_NOSPAWN=1 targets an already running server (127.0.0.1:7579), for use on a production server (pm2); spawning a new process would collide on the port.
...
// Two mock peers that record what they receive and answer json: the notification receiver and the AE's poa
...
// Values are dropped; only the key structure and types are kept, arrays as length plus the structure of the first element
function shapeOf(v) {
...
(async () => {
    ...
    try {
        ...
        // A PUT with an hd:* root is rejected by the gate (ty comparison) with 400-42. The real update path is the m2m:fcnt root: update_action picks HD_UPDATE from the stored cnd.
        ...
        // moduleclass attributes (lvl etc.) are not in the fcnt attribute list, so 400. HD_UPDATE is reached through an fcnt common attribute (lbl): update_action picks update_hd_* from the stored cnd. The response root is root_key(fcnt + cnd), i.e. 'hd:<short>', and the rep key of the hd:bat subscription's notification is the same.
