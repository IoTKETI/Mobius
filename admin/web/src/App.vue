...
/** The ri carried between screens: from the problem list to the detail, from the detail to the simulator. */
...
<template>
  ...
  <template v-else>
    <header>
      ...
      <!-- Always shows what the console can do. Connected as superuser means every ACP is passed, and that is not hidden. -->
    ...
    <!-- Observe mode sends denials out as allowed. Forgotten on, ACP is disabled altogether, so it is shown at the very top. -->
    ...
    <main>
      <!-- Re-read when returning to a tab. The screens run heavy scans, and cached results would present stale numbers as facts. -->
