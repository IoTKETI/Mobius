...
/** Refused because a job is already running. Carries that job so the screen can attach to it and show it. */
...
/** Returns the running job if there is one, so a freshly opened screen does not miss a job in progress. */
...
/** Builds 'YYYYMMDDThhmmss' N days from today. */
...
/** No continuation; see the server comment. When the cap is hit, only capped is reported. */
...
/** @param acpiOverride asks with unsaved state: the 'what if this ACP is detached' preview. An empty array is meaningful, so it is passed distinct from undefined. */
...
/** Only what is sent changes; passing pv alone leaves pvs as it is. */
...
/**
 * Previews the verdict with an unsaved body.
 * acpRowsOverride is an array of {ri, pv, pvs} rows.
 */
...
/** Days since expiry; negative means not yet expired. */
