...
/** acop bits; the screen must be able to tell what 63 means. */
...
/**
 * The template set defined by the operating model: use only three exceptions, do not create an ACP per resource, keep the number of ACPs in single digits. Starting from a template keeps that policy on the screen.
 * Source: docs/superpowers/specs/2026-08-29-acp-operating-model.md
 */
...
/** Whether the preview reflects the rules currently on the screen; editing makes it stale. */
...
async function load() {
  ...
  try {
    ...
    // When a resource actually uses this ACP, it is used for the preview.
...
/** Applies a template to pv. Overwrites but does not save: the preview and the save button still stand in front, so a wrong click can be undone. pvs is left alone; removing the administrator by mistake is the most expensive error. */
...
// Editing a rule re-runs the check and marks the preview as stale.
watch(
  [pv, pvs],
  () => {
    saved.value = false
...
<template>
  <section>
    ...
    <template v-if="detail">
      ...
      <!-- Start from a template instead of writing rules by hand: this is where the 'only three exceptions' policy is kept on the screen. -->
      <div class="templates">
      ...
      <!-- Check: code blocks saving, warnings do not. -->
      ...
      <!-- Preview, a step the contract requires: the console connects as superuser and never has the lock it created checked against itself. -->
      <div class="preview">
