...
const cinNote = computed(() => {
  ...
  return s.sampleTruncated
    ? `표본 ${result.value!.sampleCap.toLocaleString()}건에서 멈췄습니다 — 실제로는 더 있습니다`
    : '목록에만 있고 내용이 없습니다. 검색 결과에도 나오지 않습니다'
})
...
<template>
  <section>
    ...
    <template v-if="result">
      ...
      <h3>내용이 없는 데이터</h3>
      <!-- 예전에는 여기서 '인수인계 문서 §6 을 읽고 결정합니다' 로 끝냈다. 이 화면을 보는
           사람은 그 문서에 닿을 수 없다 — 문서가 말하던 것을 여기 옮겨 적는다. -->
      <p class="sub">
        리소스 목록에는 줄이 있는데 실제 데이터가 없는 것들입니다. 부모는 멀쩡히 있습니다 —
        부모가 없어진 것은 위의 “끊긴 지점” 입니다. 검색 결과에는 나오지 않고, 경로로 직접 열면
