...
/** Reads 'which subtree's remains' from the orphan's parent path. */
...
// ── Jobs ──
// The list is re-read after the job ends. Here that matters especially: deleting a break point makes its children new orphans, and reading right after start would show that update as 'not deleted'.
...
/* '3 rows' looks trivial while millions of rows may be unreachable; that changes the administrator's judgement, so the box is visually distinct from the explanation box. */
.caution {
