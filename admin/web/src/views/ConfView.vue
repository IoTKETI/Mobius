 ...
 */
const GROUP_ORDER = ['권한', '요청 처리', '저장소', '네트워크']
const GROUP_DESC: Record<string, string> = {
  '권한': '누가 무엇을 할 수 있는가. 잘못 두면 보호가 조용히 사라집니다.',
  '요청 처리': '들어오는 요청과 나가는 요청의 한도.',
  '저장소': '어디에 저장하고 얼마나 보관하는가.',
  // 코어 스키마에 이 묶음이 있지만 지금은 전부 exposed:false 라 화면에 안 나온다
  // (csebaseport, pxyMqttPort 등). 노출되는 날 설명 없이 나오지 않게 미리 둔다.
  '네트워크': '어느 포트로 듣는가. 바꾸면 이 주소를 아는 모든 장치가 못 붙습니다.',
}
